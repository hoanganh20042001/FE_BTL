...
export default function Purch() {
    ...
    React.useEffect(() => {
        if (localStorage.getItem("cart")) {
            ...
            console.log(id);
            axios.get(`http://localhost:8080/api/purch/${id}`)
                .then((response) => {
        ...
        <Container style={{ marginTop: 80 }} className="cart-container">
            <Grid >
                ...
                {cartList.map(item => <Grid.Col>
                    <CartPurch id={item.id} img={item.image} name={item.name} price={item.price} quantity={item.quantity} brand={item.brand} setTotal={setTotal} cartList={cartList} setCartList={setCartList} />
                </Grid.Col>)}
                {/* <Grid.Col>
